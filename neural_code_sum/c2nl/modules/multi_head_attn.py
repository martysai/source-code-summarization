...
class MultiHeadedAttention(nn.Module):
    ...
    def __init__(self, head_count, model_dim, d_k, d_v, dropout=0.1,
        ...
        if use_tree_relative_attn in {"mult1", "mult2"}:
            self.tree_relative_embeddings = nn.Embedding(tree_rel_vocab_size, \
                                                         head_count)
        elif use_tree_relative_attn == "addit":
            self.tree_relative_embeddings_k = nn.Embedding(tree_rel_vocab_size, \
                                                        self.d_k)
            self.tree_relative_embeddings_v = nn.Embedding(tree_rel_vocab_size, \
                                                        self.d_v)

    def forward(self, key, value, query, mask=None, rel_matrix=None, rel_mask=None, \
                layer_cache=None,
        ...
        ##### tree relative attention #####
        if self.use_tree_relative_attn == "addit":
            # drop_attn # batch, heads, len, len
            # tree_relation_values # batch, len, len, dim
            relmatmul = torch.matmul(drop_attn[:, :, :, None, :], \
