    ...
    def __init__(self, args):
        ...
        if self.use_type:
            self.type_embeddings = nn.Embedding(args.type_vocab_size,
                                                # len(constants.TOKEN_TYPE_MAP),
                                                self.enc_input_size)

        self.src_pos_emb = args.src_pos_emb
        self.tgt_pos_emb = args.tgt_pos_emb
        #self.no_relative_pos = all(v == 0 for v in args.max_relative_pos) or self.use_tree_relative_attn
...
class Encoder(nn.Module):
    def __init__(self,
        ...
        self.transformer = TransformerEncoder(num_layers=args.nlayers,
                                              ...
                                              always_add_type_emb=\
                                              args.always_add_type_embedding,\
...
class Transformer(nn.Module):
    ...
    def _run_forward_ml(self,
        ...
        code_rep = self.embedder(code_word_rep,
                                 ...
                                 code_type_rep,
                                 mode='encoder')
        memory_bank, layer_wise_outputs = self.encoder(code_rep, code_len, code_rel_matrix_rep, code_type_rep)  # B x seq_len x h
    ...
    def decode(self,
        ...
        word_rep = self.embedder(code_word_rep,
                                 ...
                                 code_type_rep,
                                 mode='encoder')
        memory_bank, layer_wise_outputs = self.encoder(word_rep, code_len, code_rel_matrix_rep, code_type_rep)  # B x seq_len x h
