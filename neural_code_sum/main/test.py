...
def add_test_args(parser):
    ...
    files.add_argument('--data_dir', type=str, default='my_data/',
...
def prepare_batch(batch, model, args):
...
def validate_official(args, data_loader, model, logger):
    ...
    with torch.no_grad():
        ...
        for batch_no, ex in enumerate(pbar):
            ...
            batch_inputs = prepare_batch(ex, model, args)
    ...
    if args.only_generate:
        with open(logger.path+"/preds_beam.json", 'w') as fw:
        ...
        logger.print('beam evaluation official: '
        ...
        logger.add(200000, bleu=bleu, rouge_l=rouge_l, meteor=meteor, precision=precision,\
                  recall=recall, f1=f1, examples=examples)
        logger.save()

        if args.save_pred:
            with open(log.path+"/preds_beam.json", 'w') as fw:
...
def main(args, logger):
    ...
    # --------------------------------------------------------------------------
    # DO TEST
    validate_official(args, dev_loader, model, logger)
...
def run_everything(args=None, logger=None):
    ...
    if logger is None:
        ...
        logger = logger_module.Logger(args.comment, fmt=fmt, base=args.dir, path=path)
