...
class MergeWeightMask(object):
    ...
    def build_mask_subtree_only(cls, device, num_heads, key_pad, node_pad, spans, b, t, n, m, **kwargs):
        ...
        with torch.no_grad():
            ...
            key_pad = key_pad.type(torch.BoolTensor).to(device)
            node_pad = node_pad.type(torch.BoolTensor).to(device)
            ...
            l_npad = l_npad.type(torch.BoolTensor).to(device)
...
class MergeHierarchicalEmbedding(nn.Embedding):
    ...
    def forward(self, n, spans):
        ...
        b = bh // self.num_heads
        with torch.no_grad():
            fl_mask = torch.flip(mask.type(torch.BoolTensor).to(spans.device), [2]).squeeze_(-1)
